    ...
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans text-slate-900 selection:bg-teal-100 selection:text-teal-900">
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-md transition-all duration-300">
           <div className="bg-white rounded-2xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-slate-100">
                   ...
                   <div className="space-y-4">
                       ...
                       <div className="space-y-1">
                           <label className="block text-[13px] font-bold text-slate-700 flex items-center justify-between">
                              Resume / CV 
                              <span className="text-[11px] font-medium text-slate-400 font-normal bg-slate-100 px-2 py-0.5 rounded-md">Optional</span>
                           </label>
                           ...
                           <div 
                               ...
                               className={`w-full border-2 border-dashed transition-all duration-200 rounded-xl p-5 flex flex-col items-center justify-center cursor-pointer group ${
                                  selectedFile ? 'border-teal-400 bg-teal-50/50' : 'border-slate-200 hover:border-teal-300 hover:bg-teal-50/30 bg-slate-50'
                               }`}
                           >
                               {selectedFile ? (
                                  <div className="flex flex-col items-center text-teal-700">
                                     <div className="w-10 h-10 bg-white shadow-sm rounded-lg flex items-center justify-center mb-2 group-hover:scale-105 transition-transform">
                                         <FileText className="w-5 h-5 text-teal-500" />
                                     </div>
                                     <span className="font-bold text-[13px] text-center line-clamp-1 truncate w-48">{selectedFile.name}</span>
                                     <span className="text-[11px] mt-1 text-teal-500 font-medium">Click to select a different file</span>
                                  </div>
                               ) : (
                                  <div className="flex flex-col items-center">
                                     <div className="w-10 h-10 bg-white shadow-sm rounded-lg flex items-center justify-center mb-2 text-slate-400 group-hover:text-teal-500 group-hover:scale-105 transition-all">
               ...
               <div className="px-5 py-4 border-t border-slate-100 flex items-center justify-end gap-3 bg-slate-50/50">
                   <button 
                      ...
                      disabled={isCreating}
                      className="px-4 py-2 rounded-lg font-bold text-[13px] text-slate-600 hover:bg-slate-200 transition-colors bg-slate-100"
                   >
                       Cancel
                   </button>
                   <button 
                      ...
                      className="px-5 py-2 rounded-lg font-bold text-[13px] text-white bg-teal-600 hover:bg-teal-700 transition-all disabled:opacity-50 flex items-center gap-2 shadow-md shadow-teal-600/30 hover:shadow-lg hover:shadow-teal-600/40 transform hover:-translate-y-0.5 active:translate-y-0"
                   >
        ...
        <div className="max-w-6xl mx-auto px-4 py-8 md:px-8 md:py-10 relative z-10 w-full min-h-[calc(100vh-2rem)]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                ...
                {!loading && workspaces.map(ws => {
                    ...
                    return (
                        <div key={ws.id} className={`bg-white rounded-2xl border ${isActive ? 'border-teal-500 ring-4 ring-teal-50/50 shadow-lg shadow-teal-100/50' : 'border-slate-200 hover:border-teal-300 shadow-sm hover:shadow-md hover:shadow-slate-200/50'} transition-all duration-300 p-5 flex flex-col relative group overflow-hidden`}>
                            ...
                            {isActive && (
                                <div className="absolute top-4 right-4 bg-teal-50 text-teal-700 text-[10px] uppercase font-black px-2.5 py-1 rounded-full flex items-center gap-1.5 border border-teal-100 shadow-sm">
                                    <div className="w-1.5 h-1.5 rounded-full bg-teal-500 animate-[pulse_2s_cubic-bezier(0.4,0,0.6,1)_infinite]" />
                                    Active
                                </div>
                            )}
                            ...
                            {!isActive && (
                                <button 
                                   ...
                                   className="absolute top-4 right-4 text-slate-300 hover:text-red-600 hover:bg-red-50 transition-all p-1.5 rounded-lg opacity-0 group-hover:opacity-100"
                            ...
                            <div className="flex items-start gap-4 mb-5 mt-1">
                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-extrabold text-xl shadow-sm ${isActive ? 'bg-gradient-to-br from-teal-500 to-emerald-500 text-white' : 'bg-slate-100 text-slate-600 group-hover:bg-teal-50 group-hover:text-teal-600 transition-colors'}`}>
                                ...
                                <div className="pr-12 flex-1">
                                    ...
                                    <p className="text-[12px] text-slate-600 flex items-center gap-1.5 font-medium bg-slate-50 inline-flex px-2.5 py-1 rounded-md border border-slate-200/60 shadow-sm">
                                         <Briefcase className="w-3.5 h-3.5 text-slate-400" /> {ws.role}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="mt-auto pt-4 border-t border-slate-100 w-full relative z-10">
                                <button 
                                   ...
                                   className={`w-full ${isSwitching ? 'p-1' : 'py-2.5 px-3'} rounded-lg font-bold text-[13px] flex items-center justify-center gap-2 transition-all duration-200 ${
                                      isActive ? 'bg-teal-50 text-teal-700 cursor-default ring-1 ring-inset ring-teal-200/50' :
                                      isSwitching ? 'bg-slate-900 border-none' : 
                                      'bg-slate-900 hover:bg-teal-600 text-white shadow-sm hover:shadow-md hover:shadow-teal-500/25 active:scale-[0.98]'
                                   }`}
                                >
                                   {isActive ? (
                                       ...
                                       <div className="w-full flex items-center bg-slate-900 rounded-md overflow-hidden h-8 relative">
                                          ...
                                          />
                                          <div className="relative z-10 w-full text-center text-white text-[12px] flex justify-center items-center gap-1.5 drop-shadow-md tracking-wide">
                                             <Sparkles className="w-3 h-3 animate-pulse" /> Loading Context {loadingProgress}%
                                          </div>
                                       </div>
                                   ) : (
