...
export default function AILearningAssistant() {
  ...
  const handleSaveProject = async (_project: ProjectRecommendation) => {
    const mappedProject = {
      id: _project.id || Date.now().toString(),
      title: _project.title,
      description: _project.description,
    ...
    if (user?.id) {
       try {
          ...
          showToast("Failed to save project. Try again later.");
       }
    } else {
       navigate("/signin");
    }
                            ...
                            {message.projectRecommendations.map((project) => (
                              <div key={project.id} className="bg-white rounded-xl border border-slate-200 hover:border-emerald-300 transition-all p-4 shadow-sm group cursor-pointer" onClick={() => handleStartProject(project)}>
                                ...
                                <div className="flex gap-2 pt-2">
                                  ...
                                  <button onClick={(e) => { e.stopPropagation(); handleSaveProject(project); }} className="flex-[1] py-1.5 bg-slate-100 text-slate-700 rounded-lg text-xs font-bold hover:bg-slate-200 transition flex items-center justify-center gap-1 border border-slate-200">
