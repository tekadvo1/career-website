...
// Generate some high-quality mock data based on role
const generateMockJobs = (role: string): JobListing[] => {
  const companies = ['Google', 'Microsoft', 'Stripe', 'Airbnb', 'Netflix', 'Spotify', 'Amazon', 'Meta', 'Apple', 'Datadog'];
  const locations = ['San Francisco, CA', 'New York, NY', 'Seattle, WA', 'Austin, TX', 'London, UK', 'Toronto, ON'];
  
  // Create 3-5 jobs per role
  const numJobs = Math.floor(Math.random() * 3) + 3;
  const jobs: JobListing[] = [];
  
  for (let i = 0; i < numJobs; i++) {
    const isRemote = Math.random() > 0.4;
    const isSenior = Math.random() > 0.6;
    
    jobs.push({
      id: `${role.replace(/\s+/g, '-')}-${i}-${Date.now()}`,
      role: role,
      title: `${isSenior ? 'Senior ' : ''}${role}`,
      company: companies[Math.floor(Math.random() * companies.length)],
      location: isRemote ? 'Remote' : locations[Math.floor(Math.random() * locations.length)],
      isRemote,
      salary: `$${Math.floor(Math.random() * 50 + 90)},000 - $${Math.floor(Math.random() * 50 + 150)},000`,
      postedAgo: `${Math.floor(Math.random() * 14 + 1)}d ago`,
    ...
    async function fetchTargetRoles() {
      try {
        ...
        if (data.success && data.roles && data.roles.length > 0) {
          ...
          // Generate mock jobs for the selected roles
          let allJobs: JobListing[] = [];
          data.roles.forEach((role: string) => {
            allJobs = [...allJobs, ...generateMockJobs(role)];
          });
          
          // Shuffle slightly but keep high scores near top
          allJobs.sort((a, b) => b.matchScore - a.matchScore + (Math.random() * 10 - 5));
          setJobs(allJobs);
        } else {
              ...
              filteredJobs.map((job) => (
                <div key={job.id} className="group bg-white border border-slate-200 hover:border-emerald-300 rounded-2xl p-5 shadow-sm transition-all duration-200 flex flex-col sm:flex-row gap-5">
                  ...
                  <div className="w-14 h-14 bg-slate-50 border border-slate-100 rounded-xl flex items-center justify-center shrink-0 p-3 group-hover:scale-105 transition-transform duration-300">
                    <img src={`https://logo.clearbit.com/${job.company.toLowerCase().replace(' ', '')}.com`} 
                  ...
                  <div className="shrink-0 flex sm:flex-col items-center justify-end gap-2 mt-4 sm:mt-0 sm:pl-4 sm:border-l border-slate-100">
                    <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-2.5 bg-slate-900 hover:bg-emerald-600 text-white text-sm font-bold rounded-xl transition-colors shadow-sm">
